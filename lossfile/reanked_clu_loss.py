...
class CRankedLoss(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=True):
        ...
        clu_features = clu_features.detach().cpu().numpy() #截断反向传播的梯度流。
        
        centroid = kmeans(clu_features, 8, iter=10)[0]  # 聚类结果为8类，执行次数为10次。
        # k-means最后输出的结果其实是两维的,第一维是聚类中心,第二维是损失distortion,我们在这里只取第一维,所以最后有个[0]
        appearanceGroups = vq(clu_features, centroid)[0]
        # 矢量量化，使用vq函数根据聚类中心对所有数据进行分类,vq的输出也是两维的,
        # [0]表示的是所有数据的label，是所有特征数据中各个数据属于哪一类的label
        
        allGroups = np.unique(appearanceGroups)
        
        total_loss = 0
        for group in allGroups:
            indices = np.where(appearanceGroups == group)  # 输出满足条件 (即非0) 元素的坐标，得到所有数据中输入当前group的数据的位置。
            groupfeatures = global_feat[indices]   # 对应的特征
            grouplabel = labels[indices]  # 对应的标签
