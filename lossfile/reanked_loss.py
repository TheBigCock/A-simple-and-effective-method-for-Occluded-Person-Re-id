...
def rank_loss(dist_mat, labels, margin,alpha,tval):
    ...
    for ind in range(N):
        ...
        ap_weight = torch.exp(tval * ( -1 * (alpha-margin) + dist_ap ))
        ap_weight_sum = torch.sum(ap_weight) + 1e-5
        ap_pos_val_sum = torch.sum(torch.mul(ap_is_pos,ap_weight))
        loss_ap = torch.div(ap_pos_val_sum,float(ap_weight_sum))
